...
func CustomVerifyPeerCertificate(rawCerts [][]byte, verifiedChains [][]*x509.Certificate) error {
	...
	for _, uri := range cert.URIs {
		rv, policy := identity.AllowedIdentity(uri.String())
		if rv {
			...
			if config.GetMetricsEnabled() {
				if config.GetServerSideFlag() {
					...
					operations.AuthzConnectiontClientId.WithLabelValues(config.GetIdentity().String(), uri.String(), "authorized", policy).Inc()
				}
			}
			return nil
		} else {
