...
var (
	...
	bookstorePort, _                       = strconv.Atoi(utils.GetEnv("BOOKSTORE_SVC_PORT", "8443"))
	...
	bookbuyerExpectedResponseCode          = utils.GetEnv("BOOKBUYER_EXPECTED_RESPONSE_CODE", "200")
...
var loglog = logger.NewPretty("demo")
...
func RestockBooks(amount int, headers map[string]string) {
	loglog.Info().Msgf("Restocking from book warehouse with %d books", amount)
	...
	if err != nil {
		loglog.Error().Err(err).Msgf("RestockBooks: error posting to %s", chargeAccountURL)
		return
	}

	loglog.Info().Msgf("RestockBooks: Posted to %s with headers %v", req.URL, req.Header)
	...
	for _, hdr := range interestingHeaders {
		loglog.Info().Msgf("RestockBooks (%s) adding header {%s: %s}", chargeAccountURL, hdr, getHeader(resp.Header, hdr))
	}
	loglog.Info().Msgf("RestockBooks (%s) finished w/ status: %s %d ", chargeAccountURL, resp.Status, resp.StatusCode)
}
...
func GetBooks(participantName string, meshExpectedResponseCode int, booksCount *int64, booksCountV1 *int64, booksCountV2 *int64, booksCountV3 *int64) {
	...
	for {
		...
		for url := range urlSuccessMap {
			...
			expectedResponseCode, _ := strconv.Atoi(bookbuyerExpectedResponseCode)
			...
			if responseCode == http.StatusOK {
				if url == buyBook {
					if strings.HasPrefix(identity, "bookstore-v1") {
						...
						loglog.Info().Msgf("BooksCountV1=%d", booksCountV1)
					} else if strings.HasPrefix(identity, "bookstore-v2") {
						atomic.AddInt64(booksCountV2, 1)
						atomic.AddInt64(booksCount, 1)
						loglog.Info().Msgf("BooksCountV2=%d", booksCountV2)
					} else if strings.HasPrefix(identity, "bookstore-v3") {
						atomic.AddInt64(booksCountV3, 1)
						atomic.AddInt64(booksCount, 1)
						loglog.Info().Msgf("BooksCountV3=%d", booksCountV3)
					}
				}
			}
...
func GetExpectedResponseCodeFromEnvVar(envVar, defaultValue string) int {
	...
	if err != nil {
		loglog.Fatal().Err(err).Msgf("Could not convert environment variable %s='%s' to int", envVar, expectedRespCodeStr)
	}
...
func GetRawGenerator(books interface{}) func(http.ResponseWriter, *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			loglog.Fatal().Err(err).Msg("Failed to marshal book data")
		}
		_, err = w.Write(booksRaw)
		if err != nil {
			loglog.Fatal().Err(err).Msg("Failed to write raw output")
		}
	}
}
