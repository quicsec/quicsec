...
func main() {
	...
	bookStorePurchasesV3 := &common.BookStorePurchases{}
	wg := &sync.WaitGroup{}
	...
	for {
		...
		bookStorePurchasesV3Temp := *bookStorePurchasesV3
		wg.Add(1)
		go getBookData(bookStorePurchasesV3, bookStoreV3Port, wg, errc)
		...
		bookBuyerHasChanged := bookBuyerPurchases.BooksBought-bookBuyerPurchasesTemp.BooksBought != 0 ||
			...
			bookBuyerPurchases.BooksBoughtV3-bookBuyerPurchasesTemp.BooksBoughtV3 != 0
		...
		bookThiefHasChanged := bookThiefThievery.BooksStolen-bookThiefThieveryTemp.BooksStolen != 0 ||
			...
			bookThiefThievery.BooksStolenV3-bookThiefThieveryTemp.BooksStolenV3 != 0
		...
		bookStoreV3HasChanged := bookStorePurchasesV3.BooksSold-bookStorePurchasesV3Temp.BooksSold != 0
		...
		printFunc(fmt.Sprintf(
			"bookbuyer     Books bought: %d  V1 books bought: %d  V2 books bought: %d V3 books bought: %d",
			bookBuyerPurchases.BooksBought,
			bookBuyerPurchases.BooksBoughtV1,
			bookBuyerPurchases.BooksBoughtV2,
			bookBuyerPurchases.BooksBoughtV3,
		))
		...
		printFunc(fmt.Sprintf(
			"bookthief     Books stolen: %d  V1 books stolen: %d  V2 books stolen: %d V3 books stolen: %d",
			bookThiefThievery.BooksStolen,
			bookThiefThievery.BooksStolenV1,
			bookThiefThievery.BooksStolenV2,
			bookThiefThievery.BooksStolenV3,
		))
		...
		printFunc = printYellowln
		if bookStoreV3HasChanged {
			printFunc = printGreenln
		}
		printFunc(fmt.Sprintf("bookstore v3  Books sold: %d", bookStorePurchasesV3.BooksSold))
